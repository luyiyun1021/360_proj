...
def create_viewport_dataloaders(filenames, path, processed_viewport_data, cluster_id):
    for filename in tqdm(filenames):
        ...
        with open(file_path, 'rb' ) as f:
            ...
            input, gt = create_viewport_dataset(data, VIEWPORT_WINDOW)
...
def create_processed_viewport_data_and_models(path):
    ...
    for i in range(TASK_NUM - 2):
        ...
        load_path = "../models/"+ VIEWPORT_MODEL_NAME + "_" + str(i) + ".pt"
        lstms[i].load_state_dict(torch.load(load_path)["model"])
        hiddens.append(lstms[i].init_hidden(BATCH_SIZE)) 
            
    return lstms, hiddens, processed_viewport_data, user_cluster
...
def predict_viewport_lr(b_x):
    b_x = b_x.cpu().numpy()[0]
    b_x = np.transpose(b_x)
    time_window = np.array([i for i in range(VIEWPORT_WINDOW)])
    b1, a1 = estimate_coef(time_window, b_x[0])
    b2, a2 = estimate_coef(time_window, b_x[1])
    prediction = [a1 * VIEWPORT_WINDOW + b1, a2 * VIEWPORT_WINDOW + b2]
    prediction = [find_nearest(prediction[0], "w"), find_nearest(prediction[1], "h")]
    return prediction
